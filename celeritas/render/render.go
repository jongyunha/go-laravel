...
func (c *Render) GoPage(w http.ResponseWriter, r *http.Request, view string, data interface{}) error {
	myTemplate, err := template.ParseFiles(fmt.Sprintf("%s/views/%s.page.tmpl", c.RootPath, view))
	if err != nil {
		return err
	}
...
func (c *Render) JetPage(w http.ResponseWriter, r *http.Request, view string, variables interface{}, data interface{}) error {
	var vars jet.VarMap
